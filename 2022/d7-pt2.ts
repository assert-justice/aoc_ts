...
function sillySize(top: Map<string,Dir>):void{
    ...
    const topDir = top.get('/')
    if(!topDir) throw 'whoops';
    console.log(topDir.size);
    
    const spaceNeeded = minSpace - (totalSpace - topDir.size);
    // console.log(spaceNeeded);
    // console.log(totalSpace - topDir.size);
    
    smallDirs.sort((a,b) => a[1] - b[1]);
    console.log(smallDirs);
    const idx = smallDirs.findIndex(([_, size]) => size > spaceNeeded);
    console.log(idx);
    
    console.log(smallDirs[idx]);
    
    // console.log(smallDirs.(([name, size]) => size ));
